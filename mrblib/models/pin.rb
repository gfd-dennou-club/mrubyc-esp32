...
class Pin
  ...
  def setmode(mode)
    ...
    when Pin::OUT then
      ...
      puts "Pin output mode #{@pin}"
    when Pin::IN then
      gpio_set_mode_input(@pin)
      puts "Pin input mode #{@pin}"
    when Pin::OPEN_DRAIN then
      gpio_set_mode_open_drain(@pin)
      puts "Pin open_drain mode #{@pin}"
    end
  end
